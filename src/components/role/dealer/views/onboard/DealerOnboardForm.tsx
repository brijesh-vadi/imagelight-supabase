...
import { onboardDealer } from "@/actions/dealer/onboard.action";
...
const STEP_FIELDS: Record<number, (keyof DealerOnboardingForm)[]> = {
  1: [
    "companyName",
    "contactPerson",
    "gstNumber",
  ...
  2: ["companyLogo", "verificationDocument"],
};
...
const DealerOnboardForm = ({ userId, open, onOpenChange }: Props) => {
  ...
  const onSubmit = async (data: DealerOnboardingForm) => {
    try {
      setIsSubmitting(true);
      const response = await onboardDealer({ userId, data });
